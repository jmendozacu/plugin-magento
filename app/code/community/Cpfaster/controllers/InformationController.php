...
class Cpfaster_InformationController extends Mage_Core_Controller_Front_Action
{        
    ...
    public function AddressAction() {
	    ...
	    $this->mp_id = $mpcart->getCpfasterCartId();
        ...
        $mpcart = Mage::getModel('cpfaster/mpcart')->load($this->mp_id);
        ...
        $block = $this->getLayout()->createBlock('cpfaster/information_form')->setTemplate('cpfaster/information/form.phtml');
	...
	parent::_construct();
        ...
        if (empty($this->order_id)) {
           $mpcart = Mage::getSingleton('cpfaster/mpcart');
    ...
    public function SuccessAction() {
        ...
        parent::_construct();
        $this->loadLayout();
        $block = $this->getLayout()->createBlock('cpfaster/information_success')->setTemplate('cpfaster/information/success.phtml');
        $this->getLayout()->getBlock('content')->append($block);
        $root = $this->getLayout()->getBlock('root');
        $template = "page/1column.phtml";
        $root->setTemplate($template);
        ...
        if ($data && $address->getId()) {
            ...
            try {
                $address->implodeStreetAddress()->save();
                  $this->_redirect('cpfaster/information/success/');
                return;
               ...
               $this->_redirect('cpfaster/information/address');
            }
              $this->_redirect('cpfaster/information/address');
        } else {
             $this->_redirect('cpfaster/information/address');
