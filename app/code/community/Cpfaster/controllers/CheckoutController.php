...
class Cpfaster_CheckoutController extends Mage_Core_Controller_Front_Action {
    ...
    public function _construct() {
        ...
        $this->_model = Mage::getModel('cpfaster/Faster');
    ...
    public function addcartAction() {
        ...
        try{
            ...
            $this->_redirect('cpfaster/checkout/error');
        }
        if($saved){
    ...
    public function zipcodeAction() {
        ...
        if($askzip == 1) {
            if ($quote->getPostcode() == null || $quote->getPostcode() == '') {
                ...
                $this->_redirect('cpfaster/checkout/shipping');
            }
        } else {
            $cep = "-";
    ...
    public function shippingAction() {
        ...
        if ($quote->getShippingMethod() != null && $quote->getShippingMethod() != "") {
            $this->_redirect('cpfaster/checkout/cart');
        }
    ...
    public function redirectAction(){
        ...
        switch ($mode):
            case 'lightbox':
                $block = $this->getLayout()->createBlock('cpfaster/checkout_lightbox');
                $this->getLayout()->getBlock('content')->append($block);
                break;
            case 'iframe':
                $block = $this->getLayout()->createBlock('cpfaster/checkout_iframe');
                $this->getLayout()->getBlock('content')->append($block);
                $root = $this->getLayout()->getBlock('root');
                $template = "cpfaster/1column.phtml";
                $root->setTemplate($template);
                break;
            case 'redirect':
                $block = $this->getLayout()->createBlock('cpfaster/checkout_redirect');
                $this->getLayout()->getBlock('content')->append($block);
                break;
            default:
                $block = $this->getLayout()->createBlock('cpfaster/checkout_lightbox');
