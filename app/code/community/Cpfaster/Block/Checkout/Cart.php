...
class CPfaster_Block_Checkout_Cart extends Mage_Core_Block_Template
{
    ...
    private function setMpCart(){
        ...
        // save mp cart
        $mpcart = Mage::getModel('cpfaster/mpcart');
        $mpcart->setHash($hash); 
        $mpcart->setPostalCode($postalcode); 
        $mpcart->setCartId($cartid);
        $mpcart->save();
        $mpcart_id = $mpcart->getCpfaster_cart_id();
        ...
        $excludes = $faster->getConfigData('excluded_payment_methods');
        ...
        $redirectURL = $baseUrl . 'cpfaster/information/address';
        ...
        $dados = array(
            'installments' => (int) $faster->getConfigData('installments'),
            "external_reference" => 'magentoexpertcart-'.$hash ,// seu codigo de referencia, i.e. Numero do pedido da sua loja 
            "currency" => $faster->getConfigData('currency'),// string Argentina: ARS (peso argentino) ó USD (Dólar estadounidense); Brasil: BRL (Real).
            "title" => $product_name,
         ...
         $checkout = Mage::getModel('cpfaster/Checkout');
    ...
    private function Checkout($order){
        ...
        $dados = array(
            'installments' => $faster->getConfigData('installments'),
            "external_reference" => 'cpfastercart-'.$orderId ,// seu codigo de referencia, i.e. Numero do pedido da sua loja
            "currency" => $faster->getConfigData('currency'),// string Argentina: ARS (peso argentino) ó USD (Dólar estadounidense); Brasil: BRL (Real).
            "title" => $name,
