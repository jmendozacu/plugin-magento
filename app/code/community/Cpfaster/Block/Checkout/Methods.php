...
class CPfaster_Block_Checkout_Methods extends Mage_Core_Block_Template {

    public $_rates;
    public $_car;
    protected $_postpage = 'cpfaster/checkout/shippingUpdate';
    protected $_ceppage = 'cpfaster/checkout/zipcode';
    ...
    protected function _beforeToHtml() {
        $askcep = Mage::getModel('cpfaster/Faster')->getConfigData('ask_postalcode');
            ...
            foreach ($code[0] as $_rate) {
                   ...
                   $error += 1;
                    ...
                    $this->_car->getQuote()->getShippingAddress()
                                           ...
                                           ->collectTotals()
                                           ->save();
                    Mage::app()->getFrontController()->getResponse()->setRedirect(Mage::getUrl('cpfaster/checkout/cart'));
                   }
            
            
        } else {
            Mage::app()->getFrontController()->getResponse()->setRedirect(Mage::getUrl('cpfaster/checkout/noMethod'));
        }
    }
